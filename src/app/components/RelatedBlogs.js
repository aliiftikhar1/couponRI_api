        ...
        <ul>
          {relatedBlogs.map(blog => (
            <li key={blog.id} className="mb-4 flex items-start">
              <img
                src={`https://m3xtrader.com/coupon/uploads/${blog.image}`} // Replace with your actual image path
                alt={blog.title}
        ...
        <ul>
          {similarStores.map(store => (
            <li key={store.id} className="mb-4 flex items-start">
              <img
                src={`https://m3xtrader.com/coupon/uploads/${store.comp_logo}`} // Replace with your actual image path
                alt={store.com_title}
                className="w-16 h-16 object-cover rounded-lg mr-4"
              />
              <div>
        ...
        <ul>
          {similarOffers.map(offer => (
            <li key={offer.id} className="mb-4 flex items-start">
              <img
                src={`https://m3xtrader.com/coupon/uploads/${getCompanyLogo(offer.comp_id)}`} // Use the company logo based on comp_id
